...
\begin{align*}
\frac{d f}{d \textbf{x}} &= \frac{\partial f}{\partial \textbf{x}} + \frac{\partial f}{\partial A} \frac{\partial A}{\partial \textbf{x}} = \frac{\partial (\textbf{x}^T A \textbf{x}) }{\partial \textbf{x}} + \sum_{ij}\left(\frac{\partial (\textbf{x}^T A \textbf{x}) }{\partial A}\right)_{ij} \frac{\partial\textbf{x}\textbf{x}^T}{\partial\textbf{x}}
\end{align*}

The first term can be computed using vector calculus rules
\begin{align*}
\frac{\partial (\textbf{x}^T A \textbf{x}) }{\partial \textbf{x}}  = 2A\textbf{x}
\end{align*}

\begin{align*}
\frac{\partial (\textbf{x}^T A \textbf{x})}{\partial \textbf{A}} = \textbf{x}\textbf{x}^T = A
\end{align*}

From Question \questionref{q:mml55-56} c), we have

\begin{align*}
\frac{\partial A}{\partial \textbf{x}} = \textbf{x}\textbf{x}^T = A
\end{align*}
