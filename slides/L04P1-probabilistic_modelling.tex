...
\begin{itemize}
	...
	\item I.e., $x_1, ..., x_N$ are sampled i.i.d. from the same \emph{data distribution} $p_{data}(x)$
	\item However, we don't know $p_{data}(x)$
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item[3.] check if $p(x | \bm{\theta}^*)$ fits $p_{data}(x)$ well, and repeat 1 - 3 \alert{$\Leftarrow$ model checking}
...
Idea of MLE: for datapoints $x$ sampled from $p_{data}(x)$
\begin{itemize}
	\item We want to find $\bm{\theta}^*$ such that $p(x | \bm{\theta}^*) \approx p_{data}(x)$ \pause
	\only<2>{
	\item We need to measure the ``closeness'' of the two distributions $\Rightarrow$ use the KL divergence
	$$\mathrm{KL}[p_{data}(x) || p(x | \bm{\theta})] = \mathbb{E}_{p_{data}(x)} \left[ \log \frac{p_{data}(x)}{p(x | \bm{\theta})} \right]$$
	}
	\only<3>{
	\item We want this KL to be small:
	$$ \bm{\theta}^* = \arg\min_{\bm{\theta}} \mathrm{KL}[p_{data}(x) || p(x | \bm{\theta})] $$
	}
	\only<4>{
	$$ \Leftrightarrow \quad \bm{\theta}^* = \arg\max_{\bm{\theta}} \mathbb{E}_{p_{data}(x)}[\log p(x | \bm{\theta})] $$
	}
	\only<5>{
	\item Estimate using dataset $\mathcal{D} = \{x_1, ..., x_N \}$ sampled from $p_{data}(x)$: 
...
Step 3: check if $p(x | \mparam^*)$ fits $p_{data}(x)$ well
...
Datapoints $(\x, y)$ are sampled from an \emph{unknown ground truth distribution} $p_{data}(\x, y)$
...
\begin{itemize}
	...
	\item[3.] check if $p(y | \x, \bm{\theta}^*)$ fits $p_{data}(\x, y)$ well, and repeat 1 - 3
...
Idea of MLE: for datapoints $(\x, y)$ sampled from $p_{data}(\x, y)$
\begin{itemize}
	\item We want to find $\bm{\theta}^*$ such that $p(y | \x, \bm{\theta}^*) \approx p_{data}(y | \x)$ \pause
	\item We need to measure the ``closeness'' of the two distributions $\Rightarrow$ use the KL divergence
	$$\mathrm{KL}[p_{data}(y | \x) || p(y | \x, \bm{\theta})] = \mathbb{E}_{p_{data}(y | \x)} \left[ \log \frac{p_{data}(y | \x)}{p(y | \x, \bm{\theta})} \right]$$ \pause
	\item We want this KL to be small for all $\x$ sampled from $p_{data}(\x)$
	\only<3>{
	$$ \bm{\theta}^* = \arg\min_{\bm{\theta}} \mathbb{E}_{p_{data}(\x)}[\mathrm{KL}[p_{data}(y | \x) || p(y | \x, \bm{\theta})]] $$
	}
	\only<4>{
	$$ \Leftrightarrow \quad \bm{\theta}^* = \arg\max_{\bm{\theta}} \mathbb{E}_{p_{data}(\x, \y)}[\log p(y | \x, \bm{\theta})] $$
	}
	\only<5>{
	Estimate using dataset $\mathcal{D} = \{ (\x_n, y_n) \}_{n=1}^N$ from $p_{data}(\x, y)$: 
...
\begin{frame}{Probabilistic modelling: linear regression}
Step 3: check if $p(y | \x, \bm{\theta}^*)$ fits $p_{data}(y | \x)$ well
...
(See e.g., \url{https://youtu.be/xgZ8oK9Wxzg} or search relevant videos from 3Blue1Brown)
